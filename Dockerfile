...
WORKDIR /yuriRolizReceptorMicroservico
...
COPY --from=build /yuriRolizReceptorMicroservico/target/*.jar yuriRolizReceptorMicroservico.jar

# Exponha a porta que a aplicação vai usar
EXPOSE 8081

# Comando para rodar a aplicação
ENTRYPOINT ["java", "-jar", "yuriRolizReceptorMicroservico.jar"]
